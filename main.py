...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main goal: {goal};
    TASK: Write a product description that is tailored into this customer's Age group and goal. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and goal;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account goal {goal} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ..."; OUTPUT TEXT in Estonian; 
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "goal", "content"],
    template=template,
)
...
with col2:
    st.image(image='companylogo.jpg', caption='Healty and beautiful skin')
...
def get_goal():
    input_text = st.text_input(label="Customers main goal", key="goal_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, goal=goal_input, content=content_input)
